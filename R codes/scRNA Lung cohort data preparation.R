...
setwd("/Volumes/Samsung_X5/Single Cell/E-MTAB-6653/non normalised/")

# Read in `matrix.mtx`
counts <-NULL
counts <-readMM("E-MTAB-6653.aggregated_filtered_counts.mtx")
  
# Read in `genes.tsv`
genes <- read_tsv("E-MTAB-6653.aggregated_filtered_counts.mtx_rows", col_names = FALSE)
head(genes)
gene_ids <- genes$X1

# Read in `barcodes.tsv`
cell_ids <- read_tsv("E-MTAB-6653.aggregated_filtered_counts.mtx_cols", col_names = FALSE)$X1
...
# read metadata 
setwd("/Volumes/Samsung_X5/Single Cell/E-MTAB-6653/non normalised/")
meta <- data.table::fread("E-MTAB-6653.sdrf.txt")
...
metadata$region <- as.character(meta$`Characteristics[sampling site]`)[match(unlist(metadata$sample), meta$`Comment[ENA_RUN]`)]
...
setwd("/Volumes/Samsung_X5/Single Cell/E-MTAB-6653/")
saveRDS(immune.combined, "/Volumes/Samsung_X5/Single Cell/E-MTAB-6653/immune.combined Lung.rds")
